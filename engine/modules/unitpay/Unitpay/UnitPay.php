<?php
	...
	class UnitPay
	{
		...
		public function getResult()
		{
			...
			if ($params['signature'] != $this->getSha256SignatureByMethodAndParams($method, $params, \Raptor\Config::UP_SECRET_KEY))
			{
			...
			if ($method == 'check')
			{
				if(is_array($unitPayModel->getPaymentByUnitpayId($params['unitpayId'])))
				{
					return $this->getResponseSuccess('Payment already exists');
				}

				$itemsCount = floor($params['sum'] / \Raptor\Config::UP_ITEM_PRICE);

				if ($itemsCount <= 0)
				{
					return $this->getResponseError('Суммы ' . $params['sum'] . ' руб. не достаточно для оплаты товара ' .
						'стоимостью ' . \Raptor\Config::UP_ITEM_PRICE . ' руб.');
				}
