...
class ExtAPI {
	...
	public static function makereport($array) {
		if(!isset($_POST['text'])) { $answer = json_encode(array('error' => 'Cant read text')); } 
		if(!isset($_SESSION['cid'])) { $answer = json_encode(array('error' => 'Not logged in')); } 
	...
	public static function changeemail($array) {
		if(!isset($_SESSION['id'])) { $answer = json_encode(array('error' => 'Not logged in')); } 
	...
	public static function handledino($array) {
		$dino = new Dino;
		$answer = json_encode(array('answer'=>$dino->handleCommand($_REQUEST['string'])));
		return $answer;
	}
}
?>